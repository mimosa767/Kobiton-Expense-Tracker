...
const withKobitonSDK = (config, options = {}) => {
  ...
  // ── Slider header search path fix (New Arch + Codegen) ───────────────────────
  ...
  // during the Xcode build phase.  RNCSliderComponentView.mm imports these headers
  // via angle-bracket paths (#include <react/renderer/components/RNCSlider/...>),
  // but the react-native-slider pod's HEADER_SEARCH_PATHS does not include the
  // Codegen output directory, causing "file not found" errors at compile time.
  //
  // Fix: inject a post_install hook that adds $(PODS_ROOT)/../build/generated/ios
  // (== ios/build/generated/ios/) to the slider pod's HEADER_SEARCH_PATHS in every
  // Xcode build configuration.  The post_install hook modifies the Xcode project
  // directly — more reliable than pre_install which modifies the pod spec object.
  config = withDangerousMod(config, [
    'ios',
    async (mod) => {
      ...
      const GUARD = '[SliderHeaderFix v1.0]';

      if (content.includes(GUARD)) {
        console.log('[withKobitonSDK v5.1.0] SliderHeaderFix already present — skipping');
        return mod;
      }
      ...
      const FIX_RUBY = [
        `    # ${GUARD} Add Codegen output dir to react-native-slider HEADER_SEARCH_PATHS`,
        ...
        `        __shf_existing__ = __shf_bc__.build_settings['HEADER_SEARCH_PATHS'] || '$(inherited)'`,
        `        __shf_bc__.build_settings['HEADER_SEARCH_PATHS'] = [__shf_existing__, '$(PODS_ROOT)/../build/generated/ios'].join(' ')`,
        `      end`,
        `      puts "[SliderHeaderFix] Added build/generated/ios to react-native-slider HEADER_SEARCH_PATHS"`,
        `    end`,
      ].join('\n');
      ...
      if (content.includes(POST_INSTALL_MARKER)) {
        ...
        console.log('[withKobitonSDK v5.1.0] ✓ Injected SliderHeaderFix into post_install block');
      } else {
        console.warn('[withKobitonSDK v5.1.0] ⚠ post_install block not found in Podfile — SliderHeaderFix not applied');
      }
