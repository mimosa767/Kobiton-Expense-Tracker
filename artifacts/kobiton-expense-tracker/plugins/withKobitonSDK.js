...
function withSliderFix(config) {
  ...
  config = withDangerousMod(config, [
    ...
    async (mod) => {
      ...
      // STRATEGY A — Xcode project build-phase removal (PRIMARY, most reliable):
      //   Uses the CocoaPods installer object to remove RNCSliderComponentView.mm
      //   from the react-native-slider target's compile sources phase directly.
      //   This modifies the .xcodeproj in memory before it is written to disk.
      //   Works for ANY version of slider, ANY pnpm layout, regardless of what
      //   the podspec says. xcodebuild reads the .xcodeproj — if the file isn't
      //   listed in the compile sources phase, it is never compiled.
      //
      // STRATEGY B — File content stub (belt-and-suspenders):
      //   Overwrites RNCSliderComponentView.mm with an empty stub so even if
      //   something re-adds it to the build phase, it compiles to an empty object.
      const hook = `
${HOOK_MARKER}
# [KobitonFix] Remove RNCSliderComponentView.mm from the Xcode build phase.
#
# root cause: react-native-slider v5.x podspec unconditionally lists
# RNCSliderComponentView.mm in source_files. That file imports
# RNCSliderComponentDescriptor.h which is generated by Codegen only when
# slider is properly autolinking-enabled. When autolinking is excluded,
# or when Codegen does not generate the header for slider (e.g. because
# slider 5.1.x codegenConfig is not fully compatible with RN 0.81 new arch),
# the build fails with "file not found".
#
# Fix: post_install removes the file from the Xcode build phase entirely so
# xcodebuild never tries to compile it. Also overwrites the file on disk as
...
post_install do |installer|
  ...
  installer.pods_project.targets.each do |target|
    ...
    Pod::UI.puts "[KobitonSDK] Found react-native-slider target, removing Fabric component view files from build phase"
    files_to_remove = target.source_build_phase.files.select do |file|
      file.file_ref && file.file_ref.path.to_s.include?('RNCSliderComponentView')
    end
    files_to_remove.each do |file|
      Pod::UI.puts "[KobitonSDK] \u2713 Removed from compile sources: #{file.file_ref.path}"
  ...
  if removed_count > 0
    ...
    Pod::UI.puts "[KobitonSDK] \u2713 Saved Xcode project after removing #{removed_count} slider Fabric file(s)"
  else
    Pod::UI.puts '[KobitonSDK] \u2139 No RNCSliderComponentView files found in Xcode build phases (pnpm patch may have excluded them already)'
  end

  # ── Strategy B: Overwrite file content with stub ────────────────────────────
  stub_content = "// [KobitonFix] RNCSliderComponentView.mm stubbed — Fabric view excluded.\\n"
  candidates = []
  ...
  pnpm_bases.uniq.each do |pnpm_base|
    ...
    Dir.glob(File.join(pnpm_base, '@react-native-community+slider*', 'node_modules',
                       '@react-native-community', 'slider', 'ios', 'RNCSliderComponentView.mm')) { |p| candidates << p }
  end
  ...
  candidates.uniq.each do |slider_mm|
    ...
    next unless current
    next if current.include?('[KobitonFix]')
    File.write(slider_mm, stub_content)
    Pod::UI.puts "[KobitonSDK] \u2713 Stubbed file on disk: #{slider_mm}"
  end
end
`;
