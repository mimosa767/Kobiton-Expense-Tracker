...
const withKobitonSDK = (config, options = {}) => {
  ...
  // ── Slider header search path fix v2.0 (New Arch + Codegen) ─────────────────
  // With New Architecture enabled, RN Codegen generates RNCSlider C++ headers at:
  //   ios/build/generated/ios/react/renderer/components/RNCSlider/
  // during the Xcode build phase.  RNCSliderComponentView.mm imports these via
  // angle-bracket paths (#include <react/renderer/components/RNCSlider/...>).
  //
  // WHY v1.0 FAILED: setting build_settings['HEADER_SEARCH_PATHS'] in post_install
  // modifies the Xcode project's in-memory settings, but CocoaPods xcconfig files
  // (Pods/Target Support Files/react-native-slider/*.xcconfig) take precedence over
  // project-level build settings when clang compiles.  The xcconfig is what gets
  // written into the .xcresponse file passed to the compiler — our v1.0 change was
  // silently overridden by the xcconfig.
  //
  // v2.0 FIX: after pod install generates the xcconfig files, patch them directly
  // by appending $(PODS_ROOT)/../build/generated/ios to their HEADER_SEARCH_PATHS
  // line.  This is what clang actually reads.
  config = withDangerousMod(config, [
    'ios',
    async (mod) => {
      const podfilePath = path.join(mod.modRequest.platformProjectRoot, 'Podfile');
      if (!fs.existsSync(podfilePath)) {
        console.log('[withKobitonSDK v5.1.0] Podfile not found — skipping SliderHeaderFix');
        return mod;
      }

      let content = fs.readFileSync(podfilePath, 'utf8');
      const GUARD = '[SliderHeaderFix v2.0]';

      if (content.includes(GUARD)) {
        console.log('[withKobitonSDK v5.1.0] SliderHeaderFix v2.0 already present — skipping');
        return mod;
      }

      // Ruby injected into the existing post_install block.
      // Variable names prefixed with __shf__ to avoid shadowing Podfile-level vars.
      // This patches the xcconfig FILE directly — the layer clang actually reads.
      const FIX_RUBY = [
        `    # ${GUARD} Patch slider xcconfig to add Codegen output dir to HEADER_SEARCH_PATHS`,
        `    # post_install build_settings are overridden by xcconfig files — must patch xcconfig directly.`,
        `    # RNCSliderComponentView.mm needs <react/renderer/components/RNCSlider/...>`,
        `    # generated by Codegen at ios/build/generated/ios/ during the Xcode build.`,
        `    installer.pods_project.targets.each do |__shf_t__|`,
        `      next unless __shf_t__.name == 'react-native-slider'`,
        `      __shf_t__.build_configurations.each do |__shf_bc__|`,
        `        __shf_xcconfig_path__ = __shf_bc__.base_configuration_reference&.real_path`,
        `        next unless __shf_xcconfig_path__ && File.exist?(__shf_xcconfig_path__)`,
        `        __shf_content__ = File.read(__shf_xcconfig_path__)`,
        `        __shf_new_path__ = '$(PODS_ROOT)/../build/generated/ios'`,
        `        unless __shf_content__.include?(__shf_new_path__)`,
        `          if __shf_content__ =~ /HEADER_SEARCH_PATHS = (.*)/`,
        `            __shf_content__ = __shf_content__.gsub(`,
        `              /HEADER_SEARCH_PATHS = (.*)/,`,
        `              "HEADER_SEARCH_PATHS = \\\\1 #{__shf_new_path__}"`,  // double-escape: JS string → Ruby string → shell
        `            )`,
        `          else`,
        `            __shf_content__ += "\\nHEADER_SEARCH_PATHS = $(inherited) #{__shf_new_path__}\\n"`,
        `          end`,
        `          File.write(__shf_xcconfig_path__, __shf_content__)`,
        `          puts "[SliderHeaderFix v2.0] Patched xcconfig for #{__shf_bc__.name}: #{__shf_xcconfig_path__}"`,
        `        end`,
        `      end`,
        `    end`,
      ].join('\n');
      ...
      if (content.includes(POST_INSTALL_MARKER)) {
        content = content.replace(POST_INSTALL_MARKER, `${POST_INSTALL_MARKER}\n${FIX_RUBY}`);
        fs.writeFileSync(podfilePath, content);
        console.log('[withKobitonSDK v5.1.0] ✓ Injected SliderHeaderFix v2.0 (xcconfig patch) into post_install block');
      } else {
        console.warn('[withKobitonSDK v5.1.0] ⚠ post_install block not found in Podfile — SliderHeaderFix v2.0 not applied');
      }
