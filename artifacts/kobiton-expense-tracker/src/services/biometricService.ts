...
/**
 * Both platforms expose a native module named KobitonBiometricModule that wraps
 * the platform-specific Kobiton biometric SDK directly.
 *
 * iOS:    KobitonBiometricModule.m (generated by plugin) uses KobitonLAContext()
 *         — a LAContext subclass that Kobiton's runner can intercept over the
 *         internal GCDWebServer. expo-local-authentication (stock LAContext) is
 *         used only as a fallback if the native module fails to register.
 *
 * Android: KobitonBiometricModule.kt (generated by plugin) uses
 *          com.kobiton.biometric.BiometricPrompt — a BiometricPrompt subclass
 *          Kobiton's runner can intercept during test sessions.
 *
